...
def factorize_data(data, nodes):
	...
	data['Промысел'], unique = pd.factorize(data['Промысел'], na_sentinel=0)
	data['Промысел'] = np.where((data['Промысел'] == 0), 0, data['Промысел'] / len(crafts))
